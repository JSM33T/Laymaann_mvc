...
namespace Laymaann.Web.Controllers
{
    [ApiController]
    public abstract class FoundationController : ControllerBase
    {
        ...
        public FoundationController(IOptionsMonitor<LaymaannConfig> config, ILogger<FoundationController> logger, IHttpContextAccessor httpContextAccessor)
        {
            _config = config;
            _logger = logger;
        ...
        protected async Task<IActionResult> ExecuteActionAsync<T>(Func<Task<(int statusCode, T result, string message, List<string> errors)>> action, string methodName)
        {
            ...
            catch (Exception ex)
            {
                ...
                return AcResponse(500, "An error occurred while processing your request.", default(T), ["Something went wrong", "Error has ben logged"]);
            }
            finally
            {
                stopwatch.Stop();
