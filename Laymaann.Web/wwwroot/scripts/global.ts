...
//async function acPostData(apiUrl: string, data: any): APIResponse {
//    try {
//        if (!tokenElement) {
//            return { success: 401, message: 'Anti-forgery token element not found',null,null };
//        }
...
async function acPostData(apiUrl: string, data: any) {
    try {
        ...
        if (!tokenElement) {
            return { success: 401, message: 'Anti-forgery token element not found', errors: [], data: null };
        }
...
function acToastClient(titleText : string,messageText: string) {
...
function acToastServer(resp: APIResponse) {
    // long s chat gpt thing 
    let toastContainer = document.getElementById('toast-container');
    acClearToasts();
    // Create a new toast element using the DOM API
    ...
    if (resp.errors && resp.errors.length > 0) {
        ...
        errorFooter.classList.add('toast-footer','text-danger','fs-xs', 'p-1');
        ...
        errorFooter.appendChild(errorList);
        toastElement.appendChild(errorFooter);
    }
...
export {
    ...
    acToastServer, // toast (APIResponse)
    acToastClient,// toast ('type','message')
    acClearToasts,// clear toasts
    shareIt, // share logic (ID)
    InitLightGallery, // invoker for light gallery
